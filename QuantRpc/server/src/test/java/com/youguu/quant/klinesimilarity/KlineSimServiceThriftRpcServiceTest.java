...
public class KlineSimServiceThriftRpcServiceTest extends BaseTestClass {
	KlineServiceThriftRpcServiceImpl service = (KlineServiceThriftRpcServiceImpl) getBean("klineServiceThriftRpcService");
    
    @Test
    public void testFindKlineByIdTest() throws Exception {
        String json = service.findKLineSimById("600000",2222);
        System.out.println(json);
    }
    
    @Test
    public void testSaveKlineSimData() throws Exception{
    	KlineSimResult klinesim = new KlineSimResult();
    	klinesim.setName("金圆股份");
    	klinesim.setAvgDownRation(1.2);
    	klinesim.setAvgRation(1.3);
    	klinesim.setAvgUpRation(4.3);
    	klinesim.setDownRate(3.4);
    	klinesim.setId("000546");
    	...
    	String result = service.saveKlineSimResult(JSON.toJSONString(klinesim));
